...
func newRootCommand() *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		if showVersion {
			info := version.Get()
			fmt.Fprintf(cmd.OutOrStdout(), "verifier %s\n", info.Version)
			return nil
		}
...
YAML config (recommended for CI):
  verifier init chart|manifest|namespace --write verify.yaml
...
`),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 1 {
				...
				if cfgPath == "" {
					return fmt.Errorf("verifier config path is required")
				}
				if _, statErr := os.Stat(cfgPath); statErr != nil {
					return fmt.Errorf("verifier config not found: %s", cfgPath)
				}
  ...
  verifier init chart --chart ./chart --release foo -n default --write verify.yaml
  verifier verify.yaml
`)

	cmd.SetFlagErrorFunc(func(cmd *cobra.Command, err error) error {
		if err != nil {
			fmt.Fprintf(cmd.ErrOrStderr(), "Error: %v\n\nHint: generate a config with 'verifier init chart|manifest|namespace --write verify.yaml' then run 'verifier verify.yaml'\n", err)
		}
		return errUsage
	})

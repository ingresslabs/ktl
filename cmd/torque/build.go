...
import (
	...
	"github.com/ingresslabs/torque/internal/build/remote"
...
func newBuildCommandWithService(service buildsvc.Service, globalProfile *string, globalLogLevel *string, kubeconfig *string, kubeContext *string) *cobra.Command {
  ...
  torque build . --attest-dir dist/attest --policy /path/to/policy --policy-mode enforce

  # Auto-detect a compose project and build all services
  torque build ./testdata/build/compose

  # Build from a compose file directly and limit parallelism
  torque build ./testdata/build/compose/docker-compose.yml --compose-parallelism 2

  # Build with tags and push
  torque build . -f Dockerfile -t ghcr.io/acme/app:latest --push`,
	...
	cmd.Flags().Var(&validatedStringValue{dest: &opts.policyReportPath, name: "--policy-report", allowEmpty: true, validator: nil}, "policy-report", "Write a machine-readable policy report JSON to this path (defaults to --attest-dir/torque-policy-report.json when --attest-dir is set)")
	...
	cmd.Flags().Var(&validatedStringValue{dest: &opts.secretsReport, name: "--secrets-report", allowEmpty: true, validator: nil}, "secrets-report", "Write a machine-readable secrets report JSON to this path (defaults to --attest-dir/torque-secrets-report.json when --attest-dir is set)")
	...
	cmd.Flags().Var(&validatedStringValue{dest: &opts.cacheIntelOutput, name: "--cache-intel-output", allowEmpty: true, validator: nil}, "cache-intel-output", "Write cache intelligence report to this path ('-' for stdout). Defaults to stderr in human mode.")
	cmd.Flags().Var(&validatedStringValue{dest: &opts.builder, name: "--builder", validator: validateBuildkitAddr}, "builder", "BuildKit address (override with TORQUE_BUILDKIT_HOST)")
	cmd.Flags().Var(&validatedStringValue{dest: &opts.dockerContext, name: "--docker-context", allowEmpty: true, validator: nil}, "docker-context", "Docker context to use for buildx fallback (override with TORQUE_DOCKER_CONTEXT)")
	...
	cmd.Flags().Var(&validatedStringValue{dest: &opts.remoteAddr, name: "--remote-build", allowEmpty: true, validator: validateRemoteAddr}, "remote-build", "Execute this build via a remote torque-agent gRPC endpoint (host:port)")
...
var errMissingBuildContext = errors.New("'torque build' requires 1 argument (CONTEXT). Try '.' for the current directory")
...
func requireBuildContextArg(_ *cobra.Command, args []string) error {
	...
	case len(args) > 1:
		return fmt.Errorf("'torque build' accepts exactly one context argument, received %d", len(args))
	default:
		return nil
	}
}

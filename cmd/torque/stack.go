...
func newStackCommand(kubeconfig *string, kubeContext *string, logLevel *string, remoteAgent *string) *cobra.Command {
	...
	// and/or be provided via env vars. Flags remain supported (including `TORQUE_<FLAG>`), but are hidden.
	...
	// Ensure `torque stack --help` shows the full command surface (and still includes global flags).
	...
	cmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		// Important: the repo already uses TORQUE_CONFIG for the global config file path.
		// The CLI env binding layer may set this flag from that env var even when the
		// user did not intend to target `torque stack`. Only honor --config when it was
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if strings.TrimSpace(bundlePath) != "" {
				...
				tmpDir, err := os.MkdirTemp("", "torque-stack-plan-bundle-*")
				if err != nil {
					return err
				}
				...
				att := map[string]any{
					"apiVersion":         "torque.dev/stack-seal-attestation/v1",
					"planHash":           planHash,
					"planFile":           "plan.json",
				...
				fmt.Fprintf(cmd.ErrOrStderr(), "torque stack plan: wrote bundle %s (planHash=%s)\n", wrote, planHash)
				return nil
			}

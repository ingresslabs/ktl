...
func TestShipCommandCoversEveryRegisteredFlag(t *testing.T) {
	...
	covered := map[string]bool{
		...
		"sbom":                  true,
		"s3-cache":              true,
		"s3-cache-endpoint-url": true,
		"s3-cache-mode":         true,
		"s3-cache-name":         true,
		"s3-cache-path-style":   true,
		"s3-cache-region":       true,
		"secret-config":         true,
...
func TestShipCommandEveryOptionFlowsToSteps(t *testing.T) {
	...
	cmd.SetArgs([]string{
		...
		"--authfile", "auth.json",
		"--s3-cache", "s3://torque-cache/ship/api",
		"--s3-cache-region", "us-east-1",
		"--s3-cache-name", "ship-api",
		"--s3-cache-mode", "max",
...
func TestShipStepArgsCarryEvidence(t *testing.T) {
	opts := shipOptions{
		...
		s3Cache:          "s3://torque-cache/ship/api",
		s3CacheRegion:    "us-east-1",
		s3CacheName:      "ship-api",
		s3CacheMode:      "max",
		s3CacheEndpoint:  "https://s3.example.test",
		s3CachePathStyle: true,
		push:             true,
		attest:           true,
	...
	buildArgs := shipBuildArgs(opts, paths)
	for _, want := range []string{"--push", "--sbom", "--provenance", "--attest-dir", "attest", "--s3-cache", "s3://torque-cache/ship/api", "--s3-cache-region", "us-east-1", "--s3-cache-name", "ship-api", "--s3-cache-mode", "max", "--s3-cache-endpoint-url", "https://s3.example.test", "--s3-cache-path-style", "--capture=build.sqlite"} {
		if !argPresent(buildArgs, want) {
			t.Fatalf("expected build args to contain %q, got %#v", want, buildArgs)
		}
	}

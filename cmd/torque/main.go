...
func newRootCommandWithBuildService(buildService buildsvc.Service) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:          "torque plans, verifies, applies, captures, and observes Kubernetes deploy workflows.",
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			if kubeLogLevel == 0 {
				if val := strings.TrimSpace(os.Getenv("TORQUE_KUBE_LOG_LEVEL")); val != "" {
					...
					// Enable HTTP request/response tracing in client-go (DebugWrappers) when torque runs in debug mode.
					kubeLogLevel = 6
				}
			}
			if kubeLogLevel > 0 {
	...
	cmd.PersistentFlags().StringVar(&remoteAgentAddr, "remote-agent", "", "Forward torque logs and deploy operations to a remote torque-agent gRPC endpoint")
	...
	cmd.PersistentFlags().BoolVar(&remoteTLSInsecureSkipVerify, "remote-tls-insecure-skip-verify", false, "Skip TLS verification for remote gRPC (also via TORQUE_REMOTE_TLS_INSECURE_SKIP_VERIFY=1)")
	cmd.PersistentFlags().StringVar(&remoteTLSCA, "remote-tls-ca", "", "CA bundle PEM file for remote gRPC TLS (also via TORQUE_REMOTE_TLS_CA)")
	cmd.PersistentFlags().StringVar(&remoteTLSClientCert, "remote-tls-client-cert", "", "Client certificate PEM file for remote gRPC mTLS (also via TORQUE_REMOTE_TLS_CLIENT_CERT)")
	cmd.PersistentFlags().StringVar(&remoteTLSClientKey, "remote-tls-client-key", "", "Client private key PEM file for remote gRPC mTLS (also via TORQUE_REMOTE_TLS_CLIENT_KEY)")
	cmd.PersistentFlags().StringVar(&remoteTLSServerName, "remote-tls-server-name", "", "Override remote gRPC TLS server name (also via TORQUE_REMOTE_TLS_SERVER_NAME)")
	...
	cmd.Example = `  # Tail checkout pods in prod-payments and highlight errors
	  torque logs 'checkout-.*' --namespace prod-payments --highlight ERROR
...
Subcommands:
{{- range $i, $n := (list "init" "build" "ship" "apply" "delete" "stack" "revert" "repair" "replay" "list" "lint" "logs" "env" "guardian" "secrets" "security" "version") }}
...
func registerViperCommands(commands ...*cobra.Command) {
	...
	viperInitOnce.Do(func() {
		...
		v.SetEnvPrefix("TORQUE")
		v.AutomaticEnv()

		cobra.OnInitialize(func() {
			configFile := os.Getenv("TORQUE_CONFIG")
...
func handleError(err error) {
	...
	case apierrors.IsForbidden(err):
		message = fmt.Sprintf("%s\nHint: missing Kubernetes permissions. See docs/rbac.md for the verbs torque requires.", err)
	}
...
func streamFromStdin(ctx context.Context, opts *config.Options, in io.Reader, out io.Writer) error {
	tmpl, err := template.New("torque-stdin").Parse(opts.Template)
	if err != nil {
...
func configSearchDirs() []string {
	...
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		add(filepath.Join(xdg, "torque"))
	}
	if home, err := os.UserHomeDir(); err == nil && home != "" {
		add(filepath.Join(home, ".config", "torque"))
		add(filepath.Join(home, ".torque"))
	}
	return dirs
}

func setupProfiling() func() {
	mode := strings.ToLower(os.Getenv("TORQUE_PROFILE"))
	if mode != "startup" {
		return func() {}
	}
	ts := time.Now().UTC().Format("20060102-150405")
	cpuPath := fmt.Sprintf("torque-startup-%s.cpu.pprof", ts)
	...
	fmt.Fprintf(os.Stderr, "TORQUE_PROFILE=startup: writing CPU profile to %s\n", cpuPath)
	memPath := fmt.Sprintf("torque-startup-%s.mem.pprof", ts)
	return func() {
		...
		fmt.Fprintf(os.Stderr, "TORQUE_PROFILE=startup: writing heap profile to %s\n", memPath)
	}
}
...
func newUpCommand(kubeconfig, kubeContext *string) *cobra.Command {
	return &cobra.Command{
		Use:        "up",
		Short:      "Start a development workspace defined in torque.yaml",
		Hidden:     true,
		Deprecated: "use `torque logs`, `torque apply --ui`, or `torque stack status --follow`; this experimental workspace command is no longer part of the focused CLI surface",
		Long: `Reads a 'torque.yaml' file in the current directory and starts all defined log streams.
...
func runUp(ctx context.Context, kubeconfig, kubeContext *string) error {
	// Read torque.yaml
	data, err := os.ReadFile("torque.yaml")
	if err != nil {
		return fmt.Errorf("failed to read torque.yaml: %w", err)
	}
	...
	if err := yaml.Unmarshal(data, &cfg); err != nil {
		return fmt.Errorf("invalid torque.yaml: %w", err)
	}
...
func newWaitCommand(kubeconfig, kubeContext *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Hidden:     true,
		Deprecated: "use `torque apply --wait`, `torque stack apply` health gates, or `kubectl wait`; this experimental helper is no longer part of the focused CLI surface",
...
Example:
  torque wait my-app --for-log "Server started" --timeout 60s`,
